...
CAPTION = "Snake Game"
SCREEN_SIZE = (800, 800)
PLAY_RECT = pygame.Rect(16, 16, 768, 768) # pygame.Rect(left, top, width, height)
CELL = pygame.Rect(0, 0, 16, 16) # pygame.Rect(left, top, width, height)
BOARD_SIZE = (PLAY_RECT.width//CELL.width, PLAY_RECT.height//CELL.height)  #board size tuple (48,48)
GROWTH_PER_APPLE = 1

# create a dictionary of colors
COLORS = {"background" : (30, 40, 50),
         "walls" : pygame.Color("lightslategrey"),
         "snake" : pygame.Color("darkgreen"),
         "head" : pygame.Color("limegreen"),
...
# key binding for controlling the snake
KEY_MAPPING = {(pygame.K_LEFT, pygame.K_j) : "left",
                (pygame.K_RIGHT, pygame.K_l) : "right",
               (pygame.K_UP, pygame.K_i) : "up",
                (pygame.K_DOWN, pygame.K_k) : "down"}


class Apple(object):
    """Something edible.  Causes unrestricted growth in some animals."""
    def __init__(self, walls, snake):
        self.position = self.respawn(snake.body_set|walls) # avoid apple on wall or snake body
        self.walls = walls
...
class AnyKey(_Scene):
    ...
    def make_text(self, title):
        """Pre-render text."""
        self.main = FONTS["BIG"].render(title, True, pygame.Color("white"))
        self.main_rect = self.main.get_rect(centerx=PLAY_RECT.centerx,
                                            centery=PLAY_RECT.centery-150)
        text = "Press any key"
        self.ne_key = FONTS["SMALL"].render(text, True, pygame.Color("white"))
...
def draw_cell(surface, cell, color, offset=(0,0)):
    ...
    rect = pygame.Rect(pos, CELL.size)
    rect.move_ip(*offset)
...
def main():
    ...
    FONTS = {"BIG" : pygame.font.SysFont("helvetica", 100, True),
             "SMALL" : pygame.font.SysFont("helvetica", 50, True)}
    LEVELS = make_levels()
    Control().main_loop()
    pygame.quit()
    sys.exit()
