...
class Env(object):
    ...
    def reset(self, is_keep_pos_setting=True, seed=None, is_seperate=None):
        ...
        self.agents_pos = np.zeros(
            (self.dim_p, self.agents_num),  dtype=np.int)
    ...
    def make_world(self, agents_init=None, agents_targ=None):
        ...
        # add agents
        self.agents = [Agent(self.space_boundary, self.walk_dirs)
    ...
    def gen_targets(self):
        ...
        for agent in self.agents:
            ...
            while(flag):
                if self.is_seperate:
                    x = np.random.randint(
                        self.space_boundary[0] / 2, self.space_boundary[0])
                    y = np.random.randint(
                        self.space_boundary[0] / 2, self.space_boundary[1])
                    z = np.random.randint(
                        self.space_boundary[0] / 2, self.space_boundary[2])
                else:
                    x = np.random.randint(0, self.space_boundary[0])
                    y = np.random.randint(0, self.space_boundary[1])
                    z = np.random.randint(0, self.space_boundary[2])
